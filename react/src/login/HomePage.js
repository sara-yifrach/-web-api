...
const HomePage =() => {
    ...
    return (<>
        <div className="card">
            <Toolbar start={startContent} center={centerContent}  className="bg-gray-900 shadow-2" style={{ borderRadius: '3rem', backgroundImage: 'linear-gradient(to right, var(--bluegray-500), var(--bluegray-800))' }} />
        </div>
        <div className="card flex justify-content-center" > 
            <Button label="כניסה כמנהל" onClick={onClickLogin} severity="success" text />
            <Button onClick={onClickLogin}  label="כניסה" severity="help" text /></div>
        </>
    );
}
